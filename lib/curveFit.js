...
// a weak substitue for console.log( ()
...
matf.std_error = function(data, terms) {
  ...
  if (n > 2) {
    ...
    for (var i = 0;i < data.length;i++) {
      pr = data[i];
      a += Math.pow((matf.regress(pr[0], terms) - pr[1]), 2);
    }
    r = Math.sqrt(a / (n - 2));
  }
  return r;
...
matf.compute_coefficients = function(data,  p) {
  ...
  for (var i = 0;i < data.length;i++) {
    ...
    for (r = 1; r < rs; r++) {
      mpc[r] += Math.pow(pr[0], r);
    }
    // process RH column cells
    m[0][p] += pr[1];
    for (r = 1; r < p; r++) {
      m[r][p] += Math.pow(pr[0], r) * pr[1];
    }
  }
...
matf.test = function() {
  ...
  terms = this.compute_coefficients(data,6);
  ...
  cc = this.corr_coeff(data,terms);
  
 console.log('cc = ' + this.number_format(cc,prec,width));
  
  se = this.std_error(data,terms);
